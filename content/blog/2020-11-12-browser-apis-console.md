...
description: Посмотрим, что полезного можно почерпнуть из Console API за
  пределами `console.log` и как различаются реализации интерфейса между
  браузерами
tags:
  - API
  - Console
...
![](/assets/chrome-semantics.jpg "**Chrome**. **debug** выделен цветом, **info** не отличается от **log**, **warn** и **error** выделяются иконками и цветом и снабжены стек трейсом")

![](/assets/firefox-semantics.jpg "**Firefox**. **debug** не отличается от **log**, **info** выделен иконкой, **warn** и **error** – иконками и фоном, у **error** стек трейс")

![](/assets/safari-semantics.jpg "**Safari**. У всех своя иконка, **warn** и **error** выделены фоном, у **error** стек трейс")
...
![](/assets/chrome-filters.jpg "**Chrome**. **log** и **info** объединены в одну группу **Info**, для отображения **debug** нужно активировать группу **Verbose**. Реализация через множественный селект. По дефолту активированы все группы кроме Verbose, что, как говорит молодёжь, \\"сос мыслом\\", т.к. сообщения дебага, которые можно пропустить при разработке, исчезают из консоли")

![](/assets/firefox-filters.jpg "**Firefox**. **log** и **info** в отдельных группах **Logs** и **Info**, под **debug** так же отдельная группа **Debug**. Реализация через кнопки-тогглы. По дефолту выбраны все группы")

![](/assets/safari-filters.jpg "**Safari**. **log**, **info** и **debug** объединены в одну группу **Logs**. Реализация не позволяет выбирать несколько групп, и при выборе исчезает код. По дефолту выбрана опция **All**")
...
![](/assets/firefox-substitutions.jpg "**Firefox**. 4 варианта вывода составной строки разной степени паршивости")
...
![](/assets/chrome-styles.jpg "**Chrome**. Некоторые конторы любят оставлять подобные пасхалки для потенциальных соискателей")
...
![](/assets/chrome-dir-log.jpg "**Chrome**. **log** принял более наглядный вид чем строгий **dir**, при раскрытии идентичны")

![](/assets/firefox-dir-log.jpg "**Firefox**. Опять же **log** поприятнее, раскрытые совпадают, но **dir** сразу раскрыт")

![](/assets/safari-dir-log.jpg "**Safari**. **log** для литерала вообще не выводит дерева, **dir** аналогичен другим браузерам")
...
![](/assets/chrome-dir.jpg "**Chrome**. **log** и **dirxml** выводят разметку, **dir** – объект")

![](/assets/firefox-dir.jpg "**Firefox**. Не умеет в вывод разметки: все методы выводят представление через объект, просто результат **dir** развёрнут по умолчанию")

![](/assets/safari-dir.jpg "**Safari**. Поведение аналогично Chrome")
...
![](/assets/chrome-table.jpg "Chrome. Таблица выводится полностью, поддерживает сортировку, содержимое вложенных объектов не уточняется. Также под таблицей выводится результат метода `console.dir`")

![](/assets/firefox-table.jpg "Firefox. Таблица фиксированной высоты со скроллом, сортировки нет, со вложенными объектами можно взаимодействовать")

![](/assets/safari-table.jpg "Safari. Таблица выводится полностью, поддерживает сортировку, вложенные объекты вообще не показываются")
...
![](/assets/chrome-table-columns.jpg "Chrome. За счёт вывода только релевантных столбцов значительно снижается визуальный шум")
