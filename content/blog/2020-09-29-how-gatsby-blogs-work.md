...
![Pull request с новым md файлом и изображениями](/assets/netlify-cms-new-blog-page.jpg "CMS добавила контентные файлы в репозиторий")
...
![Интерфейс GraphiQL с запросом на получение данных по файлу поста](/assets/netlify-cms-new-blog-page.jpg "Через GraphiQL можем увидеть, что содержимое контентных файлов стало доступно для запросов")
...
1. Изображение для превью использует фрагмент из gatsby-transformer-sharp
1. fields -- это программно добавленные нами поля. О них подробнее чуть далее
...
1. Создать slug для каждой страницы -- уникальное имя, по которому её можно опознать
1. Выполнить GraphQL запрос за списком постов и программно создать для них страницы, используя slug для построения адреса и шаблон в качестве компонента
...
1. Для имён файлов добавляю даты для удобства
1. Для урлов использую поле из frontmatter с префиксом /blog/
...
1. Для каждого элемента массива создаём новую страницу через **createPage**, используя построенный шаблон в качестве компонента, сгенерированный slug в качестве адреса и передавая id в контекст -- это как раз и делает его доступным на уровне запросов, позволяя получать данные именно по запрашиваемой странице
